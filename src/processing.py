...
def filter_by_state(list_of_dates: list[dict[str, object]], state: str = "EXECUTED") -> list:
...
def sort_by_date(list_of_dates: list[dict[str, object]], is_reversed: bool = True) -> list:
    ...
    sorted_list = sorted(
        list_of_dates,
        key=lambda entry: datetime.fromisoformat(str(entry["date"])),
        reverse=is_reversed
    )
    return sorted_list
...
print(sort_by_date(list_of_dates, is_reversed=True))
